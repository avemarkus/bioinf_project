...
load("C:/Users/Fedor Logvin/Desktop/ДЗ_Биостат/VKR/My project/Data/phone_survey_data.rdata")

```

```{r}
#human <- subset_1 %>%
  #subset(ID == "00e9c1e4378c980a")
```

```{r}

subset_3 <- phone_survey_data[, c(1, 4:6, 11, 12, 28, 41:44)]
subset_3$num_people_home[subset_3$lives_alone == 1] <- 0
subset_3$under_18[subset_3$lives_alone == 1] <- 0
subset_3$over_65[subset_3$lives_alone == 1] <- 0
subset_3 <- na.omit(subset_3)
subset_3 <- subset_3[subset_3$num_people_home < 6]

subset_3 <- as.data.frame(apply(subset_3[, c(2, 4:10)], MARGIN = 2, as.factor))
...
ggplot(subset_3[num_people_home != c("4", "5")], aes(x = street_used_mask))+
    ...
    ylab("%")
ggsave("ношение маски по сожителям.pdf")

subset_3 %>%
  count(num_people_home)

chisq.test(table(mask = subset_3$street_used_mask, members = subset_3$num_people_home))
...
chisq.test(table(housemembers = subset_3$lives_alone, wash = subset_3$washing_hands_more))
...
ggplot(subset_3[num_people_home != 0], aes(x = street_used_mask))+
  geom_histogram(aes(y = ..density..), binwidth = 1, col = "black", fill = "green")+
  facet_grid(.~ over_65)

ggplot(subset_3[num_people_home != 0:1], aes(x = street_used_mask))+
  geom_histogram(aes(y = ..density..), binwidth = 1, col = "black", fill = "green")+
  facet_grid(under_18~ over_65)+
  scale_x_continuous(breaks = 0:2, labels = c("Не ходили", "Носят", "Не носят"))

chisq.test(table(partner = subset_3$over_65, mask = subset_3$street_used_mask))
...
chisq.test(table(partner = subset_3$under_18, mask = subset_3$street_used_mask))

ggplot(subset_3[num_people_home == 2 & over_65 == 0], aes(x = street_used_mask))+
  geom_histogram(aes(y = ..density..), binwidth = 1, col = "black", fill = "green")+
  facet_grid(.~ under_18)

table(partner = subset_3$under_18, people = subset_3$num_people_home)
table(partner = subset_3$over_65, people = subset_3$num_people_home)
table(children = subset_3$under_18[subset_3$num_people_home != 0:1], parents = subset_3$over_65[subset_3$num_people_home != 0:1])

```

```{r}

ggplot(subset_3[num_people_home != 0], aes(x = nosymptoms_past))+
  geom_histogram(aes(y = ..density..), binwidth = 1, col = "black", fill = "green")+
  facet_grid(.~ under_18)+
...
ggplot (subset_3[num_people_home != 0], aes(x = num_people_home, fill = factor(under_18)))+
  geom_bar(position = 'fill')

ggplot (subset_3[num_people_home != 0], aes(x = num_people_home, fill = factor(over_65)))+
  geom_bar(position = 'fill')
...
chisq.test(table(housemembers = subset_3$num_people_home, healthy = subset_3$nosymptoms_past))

```

```{r}

ggplot(subset_3, aes(x = nosymptoms_past))+
  geom_histogram(aes(y = ..density..), binwidth = 1, col = "black", fill = "green")+
  facet_grid(.~ num_people_home)+
  scale_x_continuous(breaks = 0:1)+
  xlab("Absence of symptoms (X axis) by housemembers (groups)")+
  ylab("% by group")
...
ggplot(subset_3, aes(x = street_used_mask))+
  geom_histogram(aes(y = ..density..), binwidth = 1, col = "black", fill = "green")+
  facet_grid(num_people_home ~ nosymptoms_past)+
  scale_x_continuous(breaks = 0:2)
ggsave("Plots/Безсимптомов_маски_сожители(гист).pdf")

```

```{r}

chisq.test(table(mask = subset_3$street_used_mask, healthy = subset_3$nosymptoms_past ))
...
glm (symptoms_score ~ lives_alone*age, fa_df, family = poisson(link = "log")) %>%
...
anova <- aov(data = subset_3, street_used_mask ~ num_people_home*under_18*over_65)
summary(anova)

plot(TukeyHSD(anova, which = "num_people_home"))

ggplot(subset_3[subset_3$under_18 == 1 & subset_3$over_65 == 1], aes(x = street_used_mask))+
  geom_histogram(aes(y = ..density..), binwidth = 1, col = "black", fill = "green")
...
fit <- lm(data = subset_3, street_used_mask ~ age + num_people_home)
...
logit_subset$num_people_home <-  as.factor(logit_subset$num_people_home)
logit_subset$street_used_mask <-  as.factor(logit_subset$street_used_mask)
logit_subset$under_18 <-  as.factor(logit_subset$under_18)
logit_subset$over_65 <- as.factor(logit_subset$over_65)
logit_subset$male <- as.factor(logit_subset$male)
...
predicted <- data.frame(street_used_mask_pred = predict(fit, subset_3), age = subset_3$age)

p <- plot_ly(x = subset_3$age, 
        y = subset_3$street_used_mask, 
        z = subset_3$num_people_home, 
        type = "scatter3d", 
        mode = "markers", 
        color = "spectral")

ggplotly(
  ggplot(subset_3, aes(x = age, y = num_people_home, col = factor(street_used_mask)))+
  geom_point()+
  geom_jitter()+
...
subset_3 <- subset_3[subset_3$street_used_mask != 0]
subset_3$street_used_mask[subset_3$street_used_mask == 2] <- 0
subset_3$street_used_mask <- as.factor(subset_3$street_used_mask)
subset_3$male <- as.factor(subset_3$male)
subset_3$num_people_home <- as.factor(subset_3$num_people_home)
subset_3$under_18 <- as.factor(subset_3$under_18)
subset_3$over_65 <- as.factor(subset_3$over_65)


splitted_data <- initial_split(subset_3, prop = 0.8, strata = "street_used_mask")
...
subset_3$age_group <- cut(subset_3$age, breaks = c(17, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 95), labels = c("18-25", "26-30", "31-35", "36-40", "41-45", "46-50", "51-55", "56-60", "61-65", "66-70", "71+"), ordered = T)

subset_3$nosymptoms_past <- as.factor(subset_3$nosymptoms_past)
subset_3$num_people_home <- as.ordered(subset_3$num_people_home)
...
Clust1 <- subset_3[subset_3$age_group == "18-25"]
...
Clust2 <- subset_3[subset_3$age_group == "31-35"]
...
Clust3 <- subset_3[subset_3$age_group == "31-35" & subset_3$under_18 == "1"]
...
Clust4 <- subset_3[subset_3$age > 50 ]
...
subset_4 <- phone_survey_data[, c(1, 4:8, 11, 12, 22:28, 42:46)]
subset_4$num_people_home[is.na(subset_4$num_people_home)] <- 0
subset_4$under_18[is.na(subset_4$under_18)] <- 0
subset_4$over_65[is.na(subset_4$over_65)] <- 0
subset_4$spouse_or_partner[is.na(subset_4$spouse_or_partner)] <- 0
subset_4$other_people[is.na(subset_4$other_people)] <- 0
subset_4 <- na.omit(subset_4)
subset_4$age_group <- cut(subset_4$age, breaks = c(17, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 95), labels = c("18-25", "26-30", "31-35", "36-40", "41-45", "46-50", "51-55", "56-60", "61-65", "66-70", "71+"), ordered = T)
subset_4 <- subset_4[subset_4$num_people_home < 6]
subset_4[, -4] %>%
  mutate(across(.cols = is.numeric, .fns = as.factor)) -> subset_4
...
chaid_res_2 <- chaid(nosymptoms_past ~ ., data = subset_4[, -c(1, 8:13)], control = chaid_control(minbucket = 150, minprob = 0.2))
...
Clust7 <-  subset_4[subset_4$age_group == "18-25"| subset_4$age_group == "26-30" | subset_4$age_group == "31-35"]
...
Clust8 <-  subset_4[(subset_4$age_group == "18-25"| subset_4$age_group == "26-30" | subset_4$age_group == "31-35") & subset_4$travelled == "0"]
...
Clust9 <-  subset_4[(subset_4$age_group == "18-25"| subset_4$age_group == "26-30" | subset_4$age_group == "31-35") & subset_4$travelled == "0" & subset_4$over_65 == "0"]
...
subset_5 <-  phone_survey_data[, c(4:11, 13, 21:37, 41:46)]

subset_5$num_people_home[is.na(subset_5$num_people_home)] <- 0
subset_5$under_18[is.na(subset_5$under_18)] <- 0
subset_5$over_65[is.na(subset_5$over_65)] <- 0
subset_5$spouse_or_partner[is.na(subset_5$spouse_or_partner)] <- 0
subset_5$other_people[is.na(subset_5$other_people)] <- 0
subset_5 <- subset_5[subset_5$num_people_home < 6]
subset_5 <- subset_5[subset_5$times_sick < 4]


#Covid test: 0 - no test, 1 - negative, 2 - positive

subset_5$selftested_covid_positive[is.na(subset_5$selftested_covid_positive)] <- 0
subset_5$selftested_covid <- subset_5$selftested_covid_positive + subset_5$selftested_covid
subset_5 <- subset_5[, -7]

#Old partner

subset_5$old_partner <- subset_5$under_18 + subset_5$over_65 + subset_5$spouse_or_partner + subset_5$other_people
subset_5$old_partner[subset_5$old_partner > subset_5$num_people_home] <- 1
subset_5$old_partner[(subset_5$spouse_or_partner == 0 | subset_5$over_65 == 0) | subset_5$old_partner == subset_5$num_people_home] <- 0
subset_5$old_partner[subset_5[, 32] == 3] <- 2
...
logit_subset_5 <- subset_5[, -c(10:15, 18:24, 26)]
logit_subset_5 <- logit_subset_5[logit_subset_5$street_used_mask != 0]
logit_subset_5$street_used_mask[logit_subset_5[, 7] == 2] <- 0
logit_subset_5 %>%
  mutate(across(.cols = !age, .fns = as.factor)) -> logit_subset_5
logit_subset_5$street_used_mask <- droplevels(logit_subset_5$street_used_mask)
...
lrm(street_used_mask ~ ., data = logit_subset_5[, -c(1, 8, 13, 17, 18)])

predicted <- data.frame(
  true_val = logit_subset_5$street_used_mask,
  est_val = predict(logit_model, newdata = logit_subset_5, type = "response")
)
...
predicted <- data.frame(
  ...
  est_val = predict(model_xgb, newdata = test_data)
)
...
model_list <- list(Random_Forest = model_forest, XGBoost = model_xgb, Logistic_Regression = model_glm, Rpart_DT = model_rpart)
...
model_nn <- caret::train(street_used_mask ~., data = train_data,
                          ...
                          metric = "ROC",
                          tuneGrid = avNNetGrid,
                          trControl = trainControl(method = "repeatedcv", number = 200,
                                                   classProbs = T, summaryFunction = twoClassSummary),
                          preProcess = c("center","scale","pca"))
